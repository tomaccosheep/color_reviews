...
def index(request):
	if request.method == 'POST': # if it's a post request
		...
		# try to update the review average for that item and color
		try:
		...
		# if the review average doesn't exist, create it
		...
		# check that the item's strongest color descriptor is still the same
